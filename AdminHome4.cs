...
namespace adpbank
{
    public partial class AdminHome4 : Form
    {
        public AdminHome4(string un_admin, string un_id)
        {
            InitializeComponent();
            // client username
        ...
        {
            this.Hide();
            var adminHome2 = new AdminHome2(userLabel.Text, adminID.Text);
            adminHome2.Closed += (s, args) => this.Close();
            adminHome2.Show();
        }
        ...
        private void AdminHome_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // table
                string query1 = "select *, DATE_FORMAT(tf_dt, '%Y-%m-%d') AS 'Date Created' from transfer INNER JOIN accounts_client ON transfer.client_id = accounts_client.id INNER JOIN info_client ON accounts_client.client_info = info_client.id where DATE_FORMAT(tf_dt, '%Y-%m-%d') = CURDATE() and transfer.active = '1' and accounts_client.active = '1' ORDER BY transfer.id DESC ";
