...
namespace adpbank
{
    public partial class TellerHome : Form
    {
        public TellerHome(string un_admin, string un_id)
        {
            InitializeComponent();
            // client username
        ...
        private void AdminHome_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // table
                string query1 = "select *, DATE_FORMAT(tr_dt, '%Y-%m-%d') AS 'Date Created' from transaction INNER JOIN accounts_client ON transaction.client_id = accounts_client.id where accounts_client.active = '1' ORDER BY transaction.id DESC ";
