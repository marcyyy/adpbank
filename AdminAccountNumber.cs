...
namespace adpbank
{
    public partial class AdminAccountNumber : Form
    {
        public AdminAccountNumber(string un_admin, string un_id)
        {
            InitializeComponent();
            // client username
        ...
        private void confirmBtn_Click(object sender, EventArgs e)
        {
            ...
            if (dt.Rows[0][0].ToString() == "1")
            {
                ...
                {
                    this.Hide();
                    var adminAllTransaction = new AdminAllTransaction(userLabel.Text, adminID.Text, acnCB.Text);
                    adminAllTransaction.Closed += (s, args) => this.Close();
                    adminAllTransaction.Show();
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message);
