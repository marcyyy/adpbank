...
namespace adpbank
{
    ...
    {
        public ClientHome(string un_client, string un_id)
        {
            InitializeComponent();
            // client username
        ...
        private void ClientHome_Load(object sender, EventArgs e)
        {
                ...
                try
                {
                    ...
                    // table
                    string query1 = "select * from transaction where client_id = '" + clientID.Text + "' ORDER BY transaction.id DESC ";
                    MySqlDataAdapter sda = new MySqlDataAdapter(query1, dbConn);

                    dbConn.Open();
                        DataSet ds = new DataSet();
                        sda.Fill(ds, "transactions");
                        clientView1.DataSource = ds.Tables["transactions"];
                    dbConn.Close();
        ...
        {
            this.Hide();
            var clientHome2 = new ClientHome2(userLabel.Text, clientID.Text);
            clientHome2.Closed += (s, args) => this.Close();
            clientHome2.Show();
        }
