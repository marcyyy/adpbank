...
namespace adpbank
{
    public partial class AdminHome3 : Form
    {
        public AdminHome3(string un_admin, string un_id)
        {
            InitializeComponent();
            // client username
        ...
        private void AdminHome_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // table
                string query1 = "select *, DATE_FORMAT(tr_dt, '%Y-%m-%d') AS 'Date Created' from transaction INNER JOIN info_client ON transaction.client_id = info_client.id where DATE_FORMAT(tr_dt, '%Y-%m-%d') = CURDATE() and transaction.active = '1' and transaction_type = 'Withdraw' ORDER BY transaction.id DESC ";
                MySqlDataAdapter sda = new MySqlDataAdapter(query1, dbConn);

                dbConn.Open();
                DataSet ds = new DataSet();
                sda.Fill(ds, "withdraw");
                ncTable.DataSource = ds.Tables["withdraw"];
                dbConn.Close();
