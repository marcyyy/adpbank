...
namespace adpbank
{
    ...
    {
        public AdminHome(string un_admin, string un_id)
        {
            InitializeComponent();
            // client username
        ...
        private void AdminHome_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // table
                string query1 = "select *, DATE_FORMAT(creation, '%Y-%m-%d') AS 'Date Created' from accounts_client INNER JOIN info_client ON accounts_client.client_info = info_client.id where DATE_FORMAT(creation, '%Y-%m-%d') = CURDATE() and accounts_client.active = '1' ORDER BY accounts_client.id DESC ";
